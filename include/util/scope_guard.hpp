...
template <class F> class ScopeGuard {
  ...
  public:
    ...
    constexpr ALWAYS_INLINE ScopeGuard(ScopeGuard &&rhs)
...
template <class F> constexpr ALWAYS_INLINE ScopeGuard<F> MakeScopeGuard(F f) {
...
#define SCOPE_GUARD                                                            \
    ::rcwg::util::impl::ScopeGuardOnExit() + [&]() ALWAYS_INLINE_LAMBDA
